...
class AKExcelView: UIView , UICollectionViewDelegate , UICollectionViewDataSource , UIScrollViewDelegate , UICollectionViewDelegateFlowLayout{
  ...
  func reloadDataCompleteExcelHandler(complete:@escaping () -> Void) {
      DispatchQueue.global().async {
...
//              self.setUpFrames()
   ...
   fileprivate func showHorizontalDivideShadowLayer() {
    ...
    //MARK: - 懒加载
    let dataManager : AKExcelDataManager = AKExcelDataManager()
    ...
    lazy var contentFreezeCollectionView : UICollectionView = {
